...
func (r *customObjectRepository) FindByObjName(c context.Context, orgId, objName string) (*entities.MTObject, error) {
	db, err := r.DB(c)
	db = opentracing.SetSpanToGorm(c, db)

	entity := &entities.MTObject{}
	if err = db.Where("org_id = ? AND obj_name = ?", orgId, objName).Take(entity).Error; err != nil {
		return nil, err
	}
	if err := db.Model(entity).Association("Fields").Find(&entity.Fields).Error; err != nil {
		return nil, err
